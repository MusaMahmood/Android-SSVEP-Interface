...
// File: tf_csm_welch_w128.cpp
//
// MATLAB Coder version            : 3.3
// C/C++ source code generated on  : 12-Mar-2018 10:19:33
//
...
static void filter(double b[4], double a[4], const double x[146], const double
                   zi[3], double y[146]);
static void filtfilt(const double x_in[128], double y_out[128]);
...
static void tf_csm_welch(const double signals[256], double fs, const double
...
static void fft(const double x[256], creal_T y[256])
{
  ...
  int j;
  double twid_re;
  static const double dv8[65] = { 1.0, 0.99879545620517241, 0.99518472667219693,
...
//
// Arguments    : const double x_in[128]
//                double y_out[128]
// Return Type  : void
//
static void filtfilt(const double x_in[128], double y_out[128])
{
  ...
  memcpy(&y[9], &x_in[0], sizeof(double) << 7);
  for (i = 0; i < 9; i++) {
    y[i + 137] = d0 - x_in[126 - i];
  }
  ...
  memcpy(&y_out[0], &y[9], sizeof(double) << 7);
}
  ...
  window[128], double CSM[192])
{
  ...
  mean(signals, dv6);
  b_repmat(dv6, Data_Block);

  //  Mean-shift (zeroing)
  for (i0 = 0; i0 < 256; i0++) {
    Data_Block[i0] = (signals[i0] - Data_Block[i0]) * data_taper[i0];
  }
  ...
  for (aa = 0; aa < 2; aa++) {
    for (b = 0; b <= 1 - aa; b++) {
      ...
      b_aa = aa + b;
      for (i0 = 0; i0 < 64; i0++) {
        CSM[i0 + (c << 6)] = b_Data_Block[i0 + (aa << 7)].re * b_Data_Block[i0 +
...
// Return Type  : void
//
void tf_csm_welch_w128(double X[256], float Y[192])
{
  double b_X[128];
  ...
  //  Filter:
  memcpy(&b_X[0], &X[0], sizeof(double) << 7);
  filtfilt(b_X, *(double (*)[128])&X[0]);
  memcpy(&b_X[0], &X[128], sizeof(double) << 7);
  filtfilt(b_X, *(double (*)[128])&X[128]);
  tf_csm_welch(X, 250.0, dv0, dv1);
  for (ix = 0; ix < 64; ix++) {
    for (ixstart = 0; ixstart < 3; ixstart++) {
      Y[ixstart + 3 * ix] = (float)dv1[ix + (ixstart << 6)];
    }
  }
  ...
  for (i = 0; i < 3; i++) {
    ...
    if (ixstart < 64) {
      while (ixstart + 1 < 65) {
        if (Y[i + 3 * ixstart] > b_mtmp) {
          b_mtmp = Y[i + 3 * ixstart];
        }
...
// Return Type  : void
//
void tf_csm_welch_w128_initialize()
{
  rt_InitInfAndNaN(8U);
}
...
// Return Type  : void
//
void tf_csm_welch_w128_terminate()
{
  // (no terminate code required)
}

//
// File trailer for tf_csm_welch_w128.cpp
//
// [EOF]
//
